...
def categorical_cross_entropy_loss(predicted_y, true_y):
    """Compute the categorical cross entropy loss between a vector of labels of size (N,C)
    and a vector of probabilities of same size
    ...
    if beta is not None:
        ...
        while mean_nr_significants > alpha:
            ...
            for i in range(nr_simulations):
                ...
                errors_i = compute_errors_first(loss, model, n_val, p, x_val, y_val)
                c_1_i, p_values_i, s_1_i, u_1_i = sign_test_first(alpha, beta, errors_i, n_val, p)
        ...
        c_1, p_values, s_1, u_1 = sign_test_first(alpha, beta, errors, n_test, p)
    if verbose:
        ...
        for key in c_1.keys():
            ...
            print('\t {0}% confidence interval: {1}'.format(int(100*(1-alpha)), np.round(c_1[key][1], 3)))
        print('------------------------------------------------\n'
...
def sfit_second_order(model, loss, alpha, beta, x, y, s_1, u_1, verbose=1):
    ...
    if verbose:
        ...
        for key in c_2.keys():
            ...
            print('\t {0}% confidence interval: {1}'.format(int(100*(1-alpha)), np.round(c_2[key][1], 3)))
        print('------------------------------------------------\n'
